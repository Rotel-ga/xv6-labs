...
sys_mmap(void)
{
  ...
  if((!f->readable && ((prot & (PROT_READ))))                              //源文件不可读，vma映射为可读
    || (!f->writable && (prot & PROT_WRITE) && !(flag & MAP_PRIVATE)))     //源文件不可写，vma映射为可写并且设置了将修改写回源文件
    return -1;
...
int vmaalloc(uint64 va)
{
  ...
  // 从磁盘读取文件
  uint64 file_offset = v->offset + (PGROUNDDOWN(va) - v->vastart);
  begin_op();
  ilock(v->f->ip);
  readi(v->f->ip, 0, (uint64)pa, file_offset, PGSIZE);
  iunlock(v->f->ip);
  end_op();
  ...
  if(mappages(p->pagetable, aligned_va, PGSIZE, (uint64)pa, pte_flags) < 0) {
    kfree(pa);
