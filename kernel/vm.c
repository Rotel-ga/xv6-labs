...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    //清除父进程的PTE_W位，并设置PTE_COW，表示所在页是一个写时复制页
    //如果本身不可写（只读），则不会添加这个标志位
    if (*pte & PTE_W)
      *pte = (*pte & ~PTE_W) | PTE_COW;

    flags = PTE_FLAGS(*pte);     // 获取当前父进程pte的标志位

    //将父进程映射的物理页直接map到子进程，权限和父进程保持一致（现在都是不可写）
...
uvmcowcopy(uint64 va)
{
  ...
  uint64 new = (uint64)kcopy_n_deref((void*)pa);  //获取新分配的物理页（若原本的物理页引用数为1，则获取到的还是原本的物理页）
  if(new == 0)  //内存不足的情况
    return -1;
  ...
  if(mappages(p->pagetable, PGROUNDDOWN(va), PGSIZE, new, flags) == -1)  //新映射（考虑要不要PGROUNDDOWN)
